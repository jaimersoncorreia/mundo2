'''
    Exercício Python 031: Desenvolva um programa que pergunte 
    a distância de uma viagem em Km. Calcule o preço da 
    passagem, cobrando R$0,50 por Km para viagens de até 
    200Km e R$0,45 parta viagens mais longas.
'''

distancia = float(input('Informe a distância da viagem: '))

passagem = (distancia * 0.45) if distancia > 200 else (distancia * 0.50)

print('O valor da passagem é de \033[34mR$ {:.2f}'.format(passagem))
